...
int main(int argc, char *argv[])
{
    ...
    for(i=0;i<v.size();i++)
    {
        ...
        if(i+2 < v.size())
        {
            temp.first = v[i];
            temp.second.first = v[i+1];
            temp.second.second.first = v[i+2];
            temp.second.second.second.first = "a";
            temp.second.second.second.second = "a";
    ...
    {
        if(u[w[i].second].second.second.first != "a")
        {
            cout<<"3 "<<u[w[i].second].first<<" "<<u[w[i].second].second.first<<" "<<u[w[i].second].second.second.first<<" "<<w[i].first<<"\n";
        }
        else if(u[w[i].second].second.first != "a")
        {
            cout<<"2 "<<u[w[i].second].first<<" "<<u[w[i].second].second.first<<" "<<w[i].first<<"\n";
        }
        else
        {
            cout<<"1 "<<u[w[i].second].first<<" "<<w[i].first<<"\n";
        }
    }
    in2.close();
    in.close();
